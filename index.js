...
client.on("ready", () => {
    ...
    client.user.setPresence({
        activity: {
            name: 'Dodaj mnie justfox.cf/nightlybot',
            type: "COMPETING"
        }
    })
})
client.on("message", msg => {
    ...
    if(msg.mentions.members.first().user.id != "622478358780837898") return;
    if(!msg.guild) return;
    const prefix = db.get(`${msg.guild.id}_prefix`) || "%"
    const embed = new Discord.MessageEmbed()
        .setTitle("Witaj użytkowniku!")
        .setDescription(`Moim prefixem jest ${prefix}, aby mnie dodać [kliknij tutaj!](https://justfox.cf/nightlybot)`)
